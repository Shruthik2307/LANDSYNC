...
export default function ExecutiveDashboard({ parcels }) {
  ...
  const avgConfidence = Math.round(
    parcels.reduce((acc, p) => acc + p.confidence, 0) / (totalCount || 1)
  );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl">
          ...
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-2xl font-extrabold text-white">{SYSTEM_METRICS_OVERVIEW.total_parcels}</span>
            <span className="text-xs font-semibold text-emerald-400 flex items-center">
              +12.4% <ArrowUpRight className="w-3 h-3 ml-0.5" />
            </span>
          </div>
          <div className="text-[11px] text-slate-400 mt-1">Multi-source NAKSHA Survey Zone</div>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl">
          <div className="flex items-center justify-between text-slate-400 text-xs font-medium">
            <span>Auto-Harmonization Rate</span>
            <ShieldCheck className="w-4 h-4 text-emerald-400" />
          </div>
          <div className="flex items-baseline justify-between mt-2">
            <span className="text-2xl font-extrabold text-emerald-400">{avgConfidence}%</span>
            <span className="text-xs font-semibold text-slate-300">Target 85%</span>
          </div>
          ...
          <div className="space-y-3 text-xs">
            <div>
              <div className="flex justify-between text-slate-300 mb-1">
                <span>Boundary Shift (Drone ORI vs Cadastral Geometry)</span>
                <span className="font-bold text-slate-200">42% of Flagged Records</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-slate-300 mb-1">
                <span>Area Discrepancy (&gt; 5% Variance)</span>
                <span className="font-bold text-slate-200">28% of Flagged Records</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-2">
            ...
            <div>
              <div className="flex justify-between text-slate-300 mb-1">
                <span>Non-Spatial Attribute Mismatch (Revenue Katha / Name)</span>
                <span className="font-bold text-slate-200">18% of Flagged Records</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-2">
            ...
            <div>
              <div className="flex justify-between text-slate-300 mb-1">
                <span>Utility & Road Right-of-Way Easement Encroachment</span>
                <span className="font-bold text-slate-200">12% of Flagged Records</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-2">
