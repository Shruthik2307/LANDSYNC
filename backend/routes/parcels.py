...
def _ensure_loaded() -> None:
    ...
    This allows GET /api/parcels to work without a prior POST /api/process.
    Any error during auto-load is converted to an HTTPException.
    """
    if not is_loaded():
        logger.info("[parcels] Cache empty — auto-loading sample data.")
        try:
            load_data()
...
def list_parcels() -> list:
    ...
    Returns an empty list if no parcels could be reconciled.
    """
    _ensure_loaded()
    try:
